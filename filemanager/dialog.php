...
<script>
    ...
    <script>
        ...
        // TinyMCE 8 compatibility - override apply_any to use postMessage only
        // The parent window handles closing the dialog
        (function() {
            var originalApplyAny = window.apply_any;
            window.apply_any = function(url) {
                // Always use postMessage for TinyMCE 8
                window.parent.postMessage({
                    sender: 'responsivefilemanager',
                    url: url,
                    field_id: null
                }, '*');
                // Don't call the old tinymce close method - parent handles it
            };

            var originalApplyImg = window.apply_img;
            window.apply_img = function(files, field_id) {
                if (field_id && field_id !== '') {
                    // Field ID mode - use original
                    if (originalApplyImg) originalApplyImg(files, field_id);
                } else {
                    // TinyMCE mode - use postMessage
                    var url = Array.isArray(files) ? files[0] : files;
                    if (typeof url === 'string' && url.indexOf('http') !== 0) {
                        var base = jQuery('#base_url').val() + jQuery('#cur_dir').val();
                        url = base + url;
                    }
                    window.parent.postMessage({
                        sender: 'responsivefilemanager',
            ...
            var originalApplyLink = window.apply_link;
            window.apply_link = function(files, field_id) {
                if (field_id && field_id !== '') {
                    if (originalApplyLink) originalApplyLink(files, field_id);
                } else {
                    var url = Array.isArray(files) ? files[0] : files;
                    if (typeof url === 'string' && url.indexOf('http') !== 0) {
                        var base = jQuery('#base_url').val() + jQuery('#cur_dir').val();
            ...
            var originalApplyVideo = window.apply_video;
            window.apply_video = function(files, field_id) {
                if (field_id && field_id !== '') {
                    if (originalApplyVideo) originalApplyVideo(files, field_id);
                } else {
                    var url = Array.isArray(files) ? files[0] : files;
                    if (typeof url === 'string' && url.indexOf('http') !== 0) {
                        var base = jQuery('#base_url').val() + jQuery('#cur_dir').val();
